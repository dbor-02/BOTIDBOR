...
document.addEventListener('DOMContentLoaded', () => {
    ...
    mobileNavBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            switchView(view);
            // Cerrar carrito si está abierto en móvil
            closeCart();
        });
    });
    ...
    // Función para actualizar visibilidad del carrito según el tamaño de pantalla
...
function renderSalesReport() {
        ...
        <div id="date-filters" class="bg-surface border border-border rounded-2xl p-4 md:p-6 mb-6 ${filtersVisible ? '' : 'hidden'}">
            ...
            <div class="flex flex-col sm:flex-row gap-4 items-end">
                ...
                <div class="flex gap-2 w-full sm:w-auto">
                    <button onclick="window.applyDateFilter()" class="flex-1 sm:flex-none bg-primary hover:bg-primaryHover text-white px-4 py-3 rounded-lg text-sm">
                        Aplicar
                    </button>
                    <button onclick="window.resetToToday()" class="flex-1 sm:flex-none bg-surfaceHover hover:bg-surfaceHover/80 text-textMain px-4 py-3 rounded-lg text-sm">
                        Hoy
                    </button>
                </div>
            </div>
            <div class="flex flex-wrap gap-2 mt-4">
                <button onclick="window.setLastDays(7)" class="bg-surfaceHover hover:bg-primary/20 px-4 py-2 rounded-full text-sm">7 días</button>
                <button onclick="window.setLastDays(15)" class="bg-surfaceHover hover:bg-primary/20 px-4 py-2 rounded-full text-sm">15 días</button>
                <button onclick="window.setLastDays(30)" class="bg-surfaceHover hover:bg-primary/20 px-4 py-2 rounded-full text-sm">30 días</button>
                <button onclick="window.setLastDays(90)" class="bg-surfaceHover hover:bg-primary/20 px-4 py-2 rounded-full text-sm">90 días</button>
            </div>
        </div>
    `;
    ...
    // Construir el HTML base (sin la tarjeta de período)
    let html = `
        <div class="space-y-4 md:space-y-6">
            ...
            <!-- Tarjetas de resumen principales (solo Total y N° Ventas) -->
            ...
            <div class="flex justify-center">
                <button onclick="window.toggleMoreInfo()" class="flex items-center gap-2 bg-primary/10 hover:bg-primary/20 text-primary px-6 py-3 rounded-full transition-colors">
    ...
    // Información adicional (solo si showMoreInfo es true)
    if (showMoreInfo) {
        ...
        html += `
            <!-- Tarjeta de período (ahora en más información) -->
            <div class="bg-surface border border-border rounded-xl md:rounded-2xl p-4 md:p-6">
                ...
                ${Object.entries(paymentMethods).map(([method, data]) => `
                    <div class="bg-surface border border-border rounded-xl md:rounded-2xl p-3 md:p-6">
                        <div class="flex items-center gap-1 md:gap-2 mb-1 md:mb-2">
                            <i data-lucide="${getPaymentIcon(method)}" class="w-4 h-4 md:w-5 md:h-5 ${getPaymentColor(method)}"></i>
                            <p class="text-textMuted text-xs">${data.name}</p>
                        </div>
                        <p class="text-sm md:text-xl font-bold text-primary">${formatCurrency(data.total)}</p>
                        <p class="text-xs text-textMuted mt-1">${data.count} ventas</p>
                    </div>
                `).join('')}
            </div>
            
            <!-- Dos columnas de información adicional -->
