...
function initProductModal() {
    ...
    defaultImageBtn.addEventListener('click', () => {
        ...
        if (name) {
            ...
            document.getElementById('product-image').value = `https://placehold.co/400x400/18181b/10b981?text=${imageText}`;
        } else {
...
export function renderInventory() {
    ...
    tbody.innerHTML = filtered.map(p => `
        <tr class="border-b border-border hover:bg-surfaceHover/50 transition-colors">
            ...
            <td class="px-6 py-4">
                <span class="${p.stock < 10 ? 'text-red-400' : 'text-textMain'} font-medium">${p.stock}</span>
            </td>
...
window.deleteProduct = (id) => {
    ...
    modal.innerHTML = `
        <div class="bg-surface rounded-2xl w-full max-w-md p-8 border border-border shadow-2xl">
            <h3 class="text-xl font-bold mb-2">Confirmar eliminación</h3>
            <p class="text-textMuted">¿Estás seguro de eliminar este producto? Esta acción no se puede deshacer.</p>
            <div class="flex justify-end gap-3 mt-8">
                <button class="cancel-delete px-6 py-2.5 text-sm font-medium text-textMuted hover:bg-surfaceHover rounded-xl transition-colors">Cancelar</button>
                <button class="confirm-delete px-6 py-2.5 text-sm font-medium bg-red-500 text-white rounded-xl hover:bg-red-600 transition-all">Eliminar</button>
            </div>
        </div>
    `;
