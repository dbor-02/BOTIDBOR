...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (cartSidebar && window.innerWidth < 1024) {
        ...
        document.body.classList.add('cart-open'); // Bloquear scroll del body
    }
};
...
window.closeCart = () => {
    ...
    if (cartSidebar && window.innerWidth < 1024) {
        cartSidebar.style.transform = 'translateX(100%)';
        document.body.classList.remove('cart-open'); // Restaurar scroll
    }
};
...
// Función toggleMoreInfo mejorada
...
function renderSalesReport() {
    ...
    if (showMoreInfo) {
        ...
        html += `
            ...
            <!-- Estadísticas por método de pago - CORREGIDO -->
            <div class="grid grid-cols-3 gap-2 md:gap-4">
                ${Object.entries(paymentMethods).map(([method, data]) => `
                    <div class="bg-surface border border-border rounded-xl p-3 md:p-4 text-center">
                        <div class="flex justify-center items-center gap-1 md:gap-2 mb-2">
                            <i data-lucide="${getPaymentIcon(method)}" class="w-5 h-5 ${getPaymentColor(method)}"></i>
                            <span class="text-textMuted text-xs md:text-sm">${data.name}</span>
                        </div>
                        <p class="text-sm md:text-base font-bold text-primary truncate">${formatCurrency(data.total)}</p>
