...
// Función para mostrar/ocultar filtros (GLOBAL)
...
window.applyDateFilter = function() {
    ...
    if (start && end) {
        ...
        // Actualizar el reporte pero mantener los filtros visibles
        renderSalesReport();
        ...
        <div id="date-filters" class="bg-surface border border-border rounded-2xl p-4 md:p-6 mb-6 ${filtersVisible ? '' : 'hidden'}">
            ...
            <div class="flex flex-col sm:flex-row gap-4 items-end">
                <div class="flex-1 w-full">
                    ...
                    <input type="date" id="date-start" value="${new Date().toISOString().split('T')[0]}" class="w-full bg-surfaceHover border border-border rounded-lg px-4 py-2 text-textMain text-sm">
                </div>
                <div class="flex-1 w-full">
                    <label class="block text-sm text-textMuted mb-1">Hasta</label>
                    <input type="date" id="date-end" value="${new Date().toISOString().split('T')[0]}" class="w-full bg-surfaceHover border border-border rounded-lg px-4 py-2 text-textMain text-sm">
                </div>
                <div class="flex gap-2 w-full sm:w-auto">
                    <button onclick="window.applyDateFilter()" class="flex-1 sm:flex-none bg-primary hover:bg-primaryHover text-white px-4 py-2 rounded-lg text-sm">
                        Aplicar
                    </button>
                    <button onclick="window.resetToToday()" class="flex-1 sm:flex-none bg-surfaceHover hover:bg-surfaceHover/80 text-textMain px-4 py-2 rounded-lg text-sm">
                        Hoy
                    </button>
                </div>
            </div>
            <div class="flex flex-wrap gap-2 mt-4">
                <button onclick="window.setLastDays(7)" class="bg-surfaceHover hover:bg-primary/20 px-3 py-1 rounded-full text-xs">7 días</button>
                <button onclick="window.setLastDays(15)" class="bg-surfaceHover hover:bg-primary/20 px-3 py-1 rounded-full text-xs">15 días</button>
                <button onclick="window.setLastDays(30)" class="bg-surfaceHover hover:bg-primary/20 px-3 py-1 rounded-full text-xs">30 días</button>
                <button onclick="window.setLastDays(90)" class="bg-surfaceHover hover:bg-primary/20 px-3 py-1 rounded-full text-xs">90 días</button>
            </div>
        </div>
    `;
    ...
    container.innerHTML = `
        <div class="space-y-4 md:space-y-6">
            ...
            <div class="flex justify-end">
                <button id="toggle-filters-btn" onclick="window.toggleDateFilters()" class="flex items-center gap-2 bg-surface border border-border px-3 md:px-4 py-2 rounded-lg hover:bg-surfaceHover transition-colors text-sm">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                <div class="bg-surface border border-border rounded-xl md:rounded-2xl p-4 md:p-6">
                    ...
                    <p class="text-sm md:text-xl font-bold break-words">${dateRange.start} - ${dateRange.end}</p>
                </div>
                <div class="bg-surface border border-border rounded-xl md:rounded-2xl p-4 md:p-6">
                    <p class="text-textMuted text-xs mb-1">Total</p>
                    <p class="text-lg md:text-3xl font-bold text-primary">${formatCurrency(totalPeriod)}</p>
                </div>
                <div class="bg-surface border border-border rounded-xl md:rounded-2xl p-4 md:p-6">
                    <p class="text-textMuted text-xs mb-1">Ventas</p>
                    <p class="text-lg md:text-3xl font-bold text-primary">${totalSales}</p>
                </div>
                <div class="bg-surface border border-border rounded-xl md:rounded-2xl p-4 md:p-6">
                    <p class="text-textMuted text-xs mb-1">Promedio</p>
                    <p class="text-lg md:text-3xl font-bold text-primary">${formatCurrency(promedioVenta)}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-3 md:gap-4">
                ${Object.entries(paymentMethods).map(([method, data]) => `
                    <div class="bg-surface border border-border rounded-xl md:rounded-2xl p-3 md:p-6">
                        <div class="flex items-center gap-1 md:gap-2 mb-1 md:mb-2">
                            <i data-lucide="${getPaymentIcon(method)}" class="w-3 h-3 md:w-5 md:h-5 ${getPaymentColor(method)}"></i>
                            <p class="text-textMuted text-xs">${data.name}</p>
                        </div>
                        <p class="text-sm md:text-2xl font-bold text-primary">${formatCurrency(data.total)}</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                ...
                <div class="space-y-4 md:space-y-6">
                    <div class="p-4 md:p-6 bg-surface border border-border rounded-xl md:rounded-2xl">
                        <h2 class="text-base md:text-xl font-bold mb-4 flex items-center gap-2">
